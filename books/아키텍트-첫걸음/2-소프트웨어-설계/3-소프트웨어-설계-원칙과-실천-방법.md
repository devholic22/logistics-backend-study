...
public record WorkRecord(LocalDate date, boolean isHoliday,
    ...
    public int calcOvertimeHours() {
        // 휴식 시간을 고려하여 근무 시간을 계산
        int hours = (int) Duration.between(clockIn, clockOut)
                                  .minusHours(BREAK_TIME)
                                  .toHours();
        // 휴일은 모두 초과근무로 처리
        if (isHoliday()) {
            return hours;
...
flowchart TB
    subgraph U1["Upper"]
        CALC1["OvertimePayCalculator"]
    end
    subgraph L1["Lower"]
        POL1["«interface»<br/>OvertimePayPolicy"]
        REG1["RegularGrade<br/>OvertimePayPolicy"]
        MAN1["ManagerGrade<br/>OvertimePayPolicy"]
    end
...
flowchart TB
    subgraph U2["Upper"]
        CALC2["OvertimePayCalculator"]
        POL2["«interface»<br/>OvertimePayPolicy"]
    end
    subgraph L2["Lower"]
        REG2["RegularGrade<br/>OvertimePayPolicy"]
        MAN2["ManagerGrade<br/>OvertimePayPolicy"]
    end
...
- **컨트롤러**는 클라이언트 요청을 처리하므로 **프로세스 로직**에 해당한다.
- 컨트롤러는 비즈니스 로직 계층의 **서비스가 제공하는 핵심 로직을 호출**하고, **화면 렌더링에 필요한 핵심 로직은 뷰가 담당**한다.
- 여기서 흥미로운 점이 있다. **서비스 내부에서는 프로세스 로직이 여러 동작을 조율하지만**, 컨트롤러에는 유스케이스를 수행하는 하나의 기능으로 공개된다.
