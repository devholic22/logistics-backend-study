...
- [시스템 컨텍스트 다이어그램](./3-아키텍처-설계/6-아키텍처-문서화.md#시스템-컨텍스트-다이어그램의-예)은 [ERP·WMS·OMS·TMS 연계](../wms-원리와-이해/1-wms-개요/README.md)가 시스템 경계와 정보 흐름을 어떻게 드러내는지 보여주는 사례와 연결된다.
- [품질 속성 시나리오](./3-아키텍처-설계/2-아키텍처-드라이버의-핵심-사항.md#품질-속성-시나리오--어느-수준까지인지를-적는다)는 [입고·출고·재고관리 KPI](../wms-원리와-이해/6-재고관리/8-재고관리-체크리스트와-KPI.md#입고출고-kpi와-무엇이-다른가)처럼 측정 기준으로 구체화해야 한다.
- [아키텍처는 트레이드오프](./3-아키텍처-설계/1-아키텍처-설계의-주요-개념.md#아키텍처는-트레이드오프다)라는 원칙은 [크로스도킹과 재고보관 비교](../wms-원리와-이해/7-크로스도킹/README.md#크로스도킹과-재고보관-비교)에서 비용·대응력·회전율을 함께 판단하는 방식으로 확인할 수 있다.
