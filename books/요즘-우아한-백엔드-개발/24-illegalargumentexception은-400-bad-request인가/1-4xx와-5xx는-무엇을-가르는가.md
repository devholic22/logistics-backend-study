...
## 빠른 복습
- **4xx는 클라이언트가 잘못된 요청을 보냈음**을 뜻한다. **요청을 수정하지 않으면 계속 실패하므로 자동 재시도 전략이 무의미할 수 있다.** **올바르게 수정한 뒤 재시도하면 정상 처리**된다.
...
| | 무엇을 의미하는가 | 대표 코드 | 재시도 |
| --- | --- | --- | --- |
| **4xx** | **클라이언트가 잘못된 요청을 보냈다** | **400 Bad Request · 401 Unauthorized · 404 Not Found** | **요청을 수정하지 않으면 계속 실패하므로 자동 재시도는 무의미할 수 있다.** 올바르게 고쳐 다시 보내면 처리된다 |
| **5xx** | **서버 내부 문제로 요청을 처리하지 못했다** | **500 Internal Server Error · 502 Bad Gateway · 503 Service Unavailable** | 서버가 회복되면 같은 요청이 성공할 수 있다 |

표를 읽는 법. **재시도 열이 이 구분의 실용적 의미**다. 상태 코드는 사람에게 설명하는 문구이기 전에 **클라이언트와 게이트웨이가 다음 행동을 결정하는 신호**다. 4xx를 받은 클라이언트가 그대로 재시도하는 것은 자원 낭비이고, 5xx를 받고 포기하는 것도 잘못된 반응이다.
