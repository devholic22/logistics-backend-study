...
sequenceDiagram
    ...
    W->>D: 미처리 event 조회
    W->>M: 환영 메일 발송
    M-->>W: 성공 또는 재시도 가능한 실패
```

메시지는 중복 전달될 수 있으므로 outbox 발행자와 소비자는 event ID를 기준으로 멱등하게 처리해야 한다.

## 외부 API와 DB는 하나의 로컬 트랜잭션이 아니다

Timeout, retry와 HTTP pool까지 포함한 호출 측 설계는 외부 연동과 DB Transaction을 함께 읽는다.
