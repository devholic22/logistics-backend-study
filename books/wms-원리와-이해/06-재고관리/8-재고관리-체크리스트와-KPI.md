...
> **수식 교정**
>
> 원서의 `재고실사오류수량 / 총보유재고수량 × 100`은 **재고오류율**에 해당한다. 위 표는 로케이션·제품·로트 등 정한 집계 단위마다 실물과 WMS 중 작은 수량을 **일치 수량**, 큰 수량을 **비교 대상 수량**으로 보고 합산한다. 과부족이 서로 상쇄되지 않고 결과가 0~100% 범위를 벗어나지 않는다. 비교 대상 수량의 합이 0이면 재고가 없는 조사이므로 정확도를 계산하지 않는다.
>
