...
> 원서에 없는 보충: 프리로케이션은 빈 로케이션 아무 곳에나 재고를 넣을 수 있어 공간 활용에 유리한 대신, 어디에 무엇이 몇 개 있는지를 **시스템이 전적으로 책임져야** 한다. WMS가 [로케이션 관리](../1-wms-개요/3-WMS의-주요-특징.md#1-로케이션-관리)를 하는 이유와 맞닿아 있다.
...
- 피킹존에 재고를 미리 내려두는 작업은 [재고보충](./3-재고이동과-재고보충.md#④-재고보충-replacement)에 있다.
