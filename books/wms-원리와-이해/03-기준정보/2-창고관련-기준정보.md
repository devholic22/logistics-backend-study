...
표를 읽는 법. **할당 여부** 한 줄이 [입고](../02-wms-주요-프로세스/2-입고와-적치.md)와 [재고관리](../06-재고관리/README.md) 전체와 연결된다. 적치 전 입고존의 재고를 가용재고에서 제외하는 데 이 플래그를 활용할 수 있다. 다만 가용재고는 로케이션 설정 하나로 확정되지 않고 **재고 보류·상태·LOT 조건과 수량**까지 함께 판단한다.
...
- [2장 로케이션과 존 — 존의 종류(입고존·보관존·피킹존 등)와 창고 배치](../02-wms-주요-프로세스/1-로케이션과-존.md)
