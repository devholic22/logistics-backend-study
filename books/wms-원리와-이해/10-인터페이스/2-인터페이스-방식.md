...
![직접 인터페이스](./assets/02-직접-인터페이스.svg)
...
![중계서버 인터페이스](./assets/03-중계서버-인터페이스.svg)

*[그림 10-5] 솔루션 연계 인터페이스 방식 — 원서는 양쪽 모두 양방향 점선이다. 가운데 중계서버가 이 방식의 전부다*

**"오류가 발생될 경우 대처가 비교적 용이하다"** 는 대목이 중계 방식의 실질적 이득이다. 중계 지점에서 여러 연계의 기록과 상태를 한곳에서 관찰할 수 있기 때문이다. 다만 중계 서버가 있다는 사실만으로 저장과 재전송이 보장되지는 않는다. **영속 메시징, 전달 보장, 재시도 정책, 실패 저장소**를 별도로 구성해야 장애 중에도 데이터를 보존하고 다시 처리할 수 있다. 이러한 구성이 없다면 중계 방식에서도 데이터가 유실될 수 있다.
...
표를 읽는 법. 이 목록은 **자동화 수준에 따라 정렬된 것이 아니다.** DB 접근, 데이터 형식, 문서 표준, 전송 프로토콜, 웹 호출처럼 서로 다른 층위의 수단을 함께 나열한다. 이 가운데 **File upload는 사람이 직접 파일을 올리는 수동 방식**이라 엄밀히 말해 자동 인터페이스가 아니다. 그럼에도 목록에 들어 있는 이유는 **상대가 시스템을 갖추지 못한 경우에도 데이터는 오가야 하기 때문**이다. [4절의 "수기처리"](./4-인터페이스-검토사항.md#가-수정-및-취소에-대한-검토)와 같은 성격의 현실적 타협이다.
