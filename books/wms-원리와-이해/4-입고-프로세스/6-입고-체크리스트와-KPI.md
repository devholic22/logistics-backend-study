...
<td>인당 처리수량 = 입고확정수량 / 투입인원수<br>장비당 처리수량 = 입고확정수량 / 투입장비수</td>
</tr>
<tr>
...
<td>수량당 입고비용 = 총입고비용 / 입고확정수량<br>건당 입고비용 = 총입고비용 / 입고확정건수</td>
</tr>
</tbody>
</table>
</div>
...
> 원서의 생산성 수식은 분자에 `입고확정수량`을 사용하면서 지표명을 `인당 처리건수`로 표기한다. 계산 단위를 일치시키기 위해 **인당 처리수량**과 **장비당 처리수량**으로 바로잡았다. 건수 기준 생산성이 필요하면 분자도 `입고확정건수`를 사용해야 한다.

> 단위당 입고비용은 비교 가능한 단위를 유지하도록 입고확정 **수량 또는 건수**를 분모로 사용한다. 인원이나 시간을 분모로 삼는 경우에는 각각 인당 비용·시간당 비용이라는 별도 지표로 관리해야 한다.
