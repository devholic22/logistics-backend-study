...
> **선의 의미**
>
> **실선은 재고 실물 이동**, **점선은 요청 정보 전달**을 나타낸다. 보관존은 사용하지 않는 경로임을 회색 노드로만 표시했다.
...
> **용어 주의**
>
> 이 장은 원서의 분류에 따라 부족분 재고배분을 `Merge-in-transit`으로 표기한다. 일반적인 물류 용례에서는 [2장의 설명](../02-wms-주요-프로세스/8-크로스도킹.md)처럼 **여러 공급처의 구성품을 운송 중 합류 지점에서 주문 단위로 병합하는 방식**을 뜻한다. 두 개념을 혼동하지 않도록 이 장에서는 본문 명칭인 **부족분 재고배분**을 사용한다.
