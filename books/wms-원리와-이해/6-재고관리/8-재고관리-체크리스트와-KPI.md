...
<td style="text-align:left">재고정확도 = (총보유재고수량 − Σ|실물수량 − WMS수량|) / 총보유재고수량 × 100</td>
</tr>
<tr>
...
> 수식 교정: 원서의 `재고실사오류수량 / 총보유재고수량 × 100`은 **재고오류율**에 해당한다. 정확도는 오류가 없는 비율이므로 위 표에는 `100 − 재고오류율`과 같은 식으로 바로잡았다. 품목별 과부족이 서로 상쇄되지 않도록 오류수량은 로케이션·제품·로트 등 정한 집계 단위별 차이의 **절댓값 합계**로 계산한다.

### 수식 적용 전에 기준을 고정한다

- **재고정확도 집계 단위** — 제품만 볼지, `로케이션 × 제품 × 로트`까지 볼지 먼저 정한다. 같은 제품의 A 로케이션 `−10`과 B 로케이션 `+10`을 합쳐 오류가 0인 것처럼 계산하면 위치 정확도를 놓친다.
...
- 입고·출고 쪽 대응 절은 [입고 체크리스트와 KPI](../4-입고-프로세스/6-입고-체크리스트와-KPI.md)와 [출고 체크리스트와 KPI](../5-출고-프로세스/8-출고-체크리스트와-KPI.md)다.
